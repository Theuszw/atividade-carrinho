...
// Manipulação dos dados do modelo a partir de JSON
...
modelsItem.querySelector('.model-item--name').innerHTML = item.name;
...
modelsItem.querySelector('a').addEventListener('click', (e)=> {
    ...
    setTimeout(()=> {
        c('.modelsWindowArea').styl.opacity = 1;
    }, 200);
...
// Adicionar evento de clique para adicionar o item no carrinho
c('.modelsInfo--addButton').addEventListener('click', ()=> {
    // Lógica para adicionar o item ao carrinho
    let size = parseInt(c('.modelsInfo--size.selected').getAttribute('data-key'));((item) => item.identifier == identifier);
    if(locaId > -1){
...
// Funçao para atualizar o conteúdo do carrinho
function updateCart(){
    // Atualizar o número de itens exibido no ícone do carrinho no medu
    ...
    if(cart.length >0){
        ...
        cart.map((itemCart, index)=>{
            ...
            // Determinar onome do tamanho do modelo com base no índice
            ...
            cartItem.querySelector('.cart--item--qt').innerHTML = itemCar.qt;
